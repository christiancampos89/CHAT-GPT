...
const OPENAI_API_KEY = "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";
    ...
    formPerguntaChat.addEventListener("submit", async (e) => {
        ...
        await fetch("https://api.openai.com/v1/completions", {
            ...
            .then((dados) => {
                ...
                document.getElementById('btn-resposta').value = dados.choices[0].text;
        ...
        var pergunta_chat = document.getElementById('campo-pergunta').value;
        var resposta_chat = document.getElementById('btn-resposta').value;
        //console.log(pergunta_chat);
        //console.log(resposta_chat);

        // Criar o Array de objetos
        let historico = new Array();

        // Verifica se já existe esse Array criado no LocalStorage, caso exista ele pega os dados de lá, converte e adiciona a histórico
        if(localStorage.hasOwnProperty("historico")){
            historico = JSON.parse(localStorage.getItem("historico"));
        }

        //Adiciona no final da fila a nova pergunta e a nova resposta
        historico.push({pergunta_chat, resposta_chat});

        //Seta as perguntas e respostas no localStorage
        localStorage.setItem("historico", JSON.stringify(historico));
        
        document.getElementById("conteudo").insertAdjacentHTML('beforeend', "Pergunta: " + pergunta_chat + "<br><br>Resposta: " + resposta_chat + "<br><hr>");
    });
}
